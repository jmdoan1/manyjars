// src/components/mention-popup.tsx
// Reusable mention suggestion popup component

import {
  Command,
  CommandEmpty,
  CommandGroup,
  CommandItem,
  CommandList,
} from '@/components/ui/command'
import type { MentionRow, MentionType } from '@/hooks/use-mentions'

export interface MentionPopupProps {
  rows: MentionRow[]
  highlightedIndex: number
  mentionType?: MentionType
  onSelect: (row: MentionRow) => void
  onHighlight: (index: number) => void
  className?: string
}

export function MentionPopup({
  rows,
  highlightedIndex,
  mentionType,
  onSelect,
  onHighlight,
  className = '',
}: MentionPopupProps) {
  if (rows.length === 0) return null

  return (
    <Command className={`rounded-md border border-white/20 bg-black/95 text-sm text-white shadow-lg ${className}`}>
      <CommandList>
        <CommandEmpty className="px-3 py-2 text-xs text-white/60">
          No matches.
        </CommandEmpty>
        <CommandGroup>
          {rows.map((row, index) => {
            const isActive = index === highlightedIndex

            if (row.kind === 'typed') {
              return (
                <CommandItem
                  key="typed-option"
                  value={row.label}
                  onSelect={() => onSelect(row)}
                  onMouseEnter={() => onHighlight(index)}
                  className={`flex flex-col items-start gap-0.5 data-[selected=true]:bg-unset data-[selected=true]:text-unset ${
                    isActive ? 'bg-white/20' : ''
                  }`}
                >
                  <span className="text-white">{row.label}</span>
                  <span className="text-[11px] text-white/60">
                    {row.description}
                  </span>
                </CommandItem>
              )
            }

            if (row.kind === 'priority') {
              return (
                <CommandItem
                  key={row.option.code}
                  value={row.option.token}
                  onSelect={() => onSelect(row)}
                  onMouseEnter={() => onHighlight(index)}
                  className={`flex flex-col items-start gap-0.5 data-[selected=true]:bg-unset data-[selected=true]:text-unset ${
                    isActive ? 'bg-white/20' : ''
                  }`}
                >
                  <span className="text-white">!{row.option.token}</span>
                  <span className="text-[11px] text-white/60">
                    {row.option.label} — {row.option.description}
                  </span>
                </CommandItem>
              )
            }

            // suggestion (jar/tag)
            return (
              <CommandItem
                key={row.item.id}
                value={row.item.name}
                onSelect={() => onSelect(row)}
                onMouseEnter={() => onHighlight(index)}
                data-selected={false}
                className={`flex items-center gap-2 data-[selected=true]:bg-unset data-[selected=true]:text-unset ${
                  isActive ? 'bg-white/20' : ''
                } text-white`}
              >
                <span>
                  {mentionType === 'jar' ? '@' : '#'}
                  {row.item.name}
                </span>
                {row.item.description && (
                  <span className="ml-2 text-[11px] text-white/60">
                    {row.item.description}
                  </span>
                )}
              </CommandItem>
            )
          })}
        </CommandGroup>
      </CommandList>
    </Command>
  )
}

export default MentionPopup
